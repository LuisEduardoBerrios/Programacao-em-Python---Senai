...
def inserir_perfil():
    ...
    if cpf and experiencia and objetivo and risco:
        conn = conectar()
        c = conn.cursor()
        c.execute("INSERT INTO perfil_investidor VALUES(?,?,?,?)",(cpf, experiencia, objetivo, risco))
        conn.commit()
        conn.close()
...
janela.configure(bg="#5E256E")

titulo = ctk.CTkLabel(janela, text="PERFIL DO INVESTIDOR",font=('Arial', 26, 'bold'),text_color="white")
...
btn_salvar = ctk.CTkButton(frame_botoes, text="SALVAR", fg_color="#9051a8",command=inserir_perfil)
